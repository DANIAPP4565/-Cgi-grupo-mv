...
def stable_rois_editor(img: Image.Image, rois_default: dict, key_prefix: str = "roi_stable") -> dict:
    """Editor estable de bordes sin componentes externos.

    Usa únicamente widgets nativos de Streamlit dentro de un formulario.
    Esto evita el error de componentes: "Bad message format / SessionInfo"
    y acelera la corrección porque la app no recalcula con cada movimiento
    del mouse: recalcula sólo al presionar Aplicar bordes.
    """
    w, h = img.size
    safe_key = _safe_canvas_key(key_prefix)
    state_key = f"{safe_key}_rois_stable_state"
    ...
    st.markdown(
        "<div class='guide'><b>Editor estable de bordes:</b> ajuste los bordes mirando la imagen. "
        "No usa componentes externos de clic/arrastre, por eso elimina el mensaje SessionInfo y evita demoras. "
        "Mueva los controles y presione <b>Aplicar bordes</b>.</div>",
        unsafe_allow_html=True,
    )

    st.image(draw_rois_with_guides(img, rois), caption="Vista actual de sectores activos", use_container_width=True)

    with st.form(key=f"{safe_key}_roi_form", clear_on_submit=False):
        new_rois = {}
        tabs = st.tabs(["dZ/dt", "ECG", "Fono"])
        for tab, key in zip(tabs, SIGNALS):
            with tab:
                b = rois[key]
                ...
                with c1:
                    x_pair = st.slider(
                        f"Bordes horizontales {labels[key]}",
                        min_value=0, max_value=int(w),
                        value=(int(b["x0"]), int(b["x1"])),
                        step=1,
                        key=f"{safe_key}_{key}_x_pair",
                    )
                with c2:
                    y_pair = st.slider(
                        f"Bordes verticales {labels[key]}",
                        min_value=0, max_value=int(h),
                        value=(int(b["y0"]), int(b["y1"])),
                        step=1,
                        key=f"{safe_key}_{key}_y_pair",
                    )
                new_rois[key] = clamp_roi({
                    "x0": int(x_pair[0]), "x1": int(x_pair[1]),
                    "y0": int(y_pair[0]), "y1": int(y_pair[1]),
                }, w, h)

        c_apply, c_reset = st.columns(2)
        with c_apply:
            applied = st.form_submit_button("Aplicar bordes", type="primary")
        with c_reset:
            reset = st.form_submit_button("Restablecer sectores")

    if reset:
        st.session_state[state_key] = rois_default
        rois = rois_default
        st.success("Sectores restablecidos.")
    elif applied:
        st.session_state[state_key] = new_rois
        rois = new_rois
        st.success("Bordes aplicados. La digitalización usará estos sectores.")

    st.image(draw_rois_with_guides(img, rois), caption="Sectores que se usarán para digitalizar", use_container_width=True)
    return rois
...
def canvas_select_rois(img: Image.Image, rois_default: dict, key_prefix: str = "roi_canvas") -> dict:
    """Permite ajustar sectores de forma estable desde la vista gráfica.

    Se elimina el uso de componentes externos para el clic/arrastre porque en
    Streamlit Cloud provocaban "Bad message format / SessionInfo" y hacían
    lenta la corrección.
    """
    return stable_rois_editor(img, rois_default, key_prefix=f"{_safe_canvas_key(key_prefix)}_stable")
...
def graph_adjust_cursors(chart_meta: dict, cursor_x: dict, key_prefix: str = "cursor_canvas") -> dict:
    """Editor estable de QRS/B/C/X/Y sin componentes externos.

    Los cursores se ajustan mirando el gráfico, con sliders nativos dentro de
    un formulario. La app recalcula sólo al presionar Aplicar cursores, lo que
    evita el mensaje SessionInfo y mejora mucho la velocidad.
    """
    img = Image.open(io.BytesIO(chart_meta["image_bytes"])).convert("RGB")
    safe_key = _safe_canvas_key(key_prefix)
    state_key = f"{safe_key}_cursor_positions_stable_state"
    x0, x1 = float(chart_meta["x0"]), float(chart_meta["x1"])

    incoming = {c: float(min(max(cursor_x.get(c, x0), x0), x1)) for c in CURSORS}
    if state_key not in st.session_state:
        st.session_state[state_key] = incoming
    positions = {c: float(min(max(st.session_state[state_key].get(c, incoming[c]), x0), x1)) for c in CURSORS}

    st.markdown(
        "<div class='guide'><b>Editor estable de cursores:</b> ajuste QRS, B, C, X e Y mirando el gráfico. "
        "Los controles están dentro de un formulario para evitar recálculos lentos con cada movimiento. "
        "Presione <b>Aplicar cursores</b> para actualizar el gráfico final.</div>",
        unsafe_allow_html=True,
    )
    st.image(img, caption="Vista de referencia para ubicar QRS, B, C, X e Y", use_container_width=True)

    with st.form(key=f"{safe_key}_cursor_form", clear_on_submit=False):
        new_positions = {}
        cols = st.columns(5)
        for i, c in enumerate(CURSORS):
            with cols[i]:
                st.markdown(f"<b style='color:{CURSOR_COLORS.get(c, '#000')}'>{c}</b>", unsafe_allow_html=True)
                new_positions[c] = float(st.slider(
                    f"Cursor {c}",
                    min_value=float(x0),
                    max_value=float(x1),
                    value=float(positions[c]),
                    step=1.0,
                    key=f"{safe_key}_{c}_slider",
                ))
        c_apply, c_reset = st.columns(2)
        with c_apply:
            applied = st.form_submit_button("Aplicar cursores", type="primary")
        with c_reset:
            reset = st.form_submit_button("Restablecer cursores automáticos")

    if reset:
        st.session_state[state_key] = incoming
        positions = incoming
        st.success("Cursores restablecidos a la detección automática.")
    elif applied:
        st.session_state[state_key] = new_positions
        positions = new_positions
        st.success("Cursores aplicados. El gráfico final y el Excel de deltas se recalcularán con estos valores.")
