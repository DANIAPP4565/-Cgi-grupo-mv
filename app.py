...
# - v9: desarrollador visible y eliminación de textos de sector auxiliar en la interfaz
...
def aplicar_estilos() -> None:
    st.markdown(
        ...
        .hero h1{color:#fff!important;margin:0;font-size:1.55rem}.hero p{color:#E0F2FE!important;margin:4px 0 0 0}.hero .dev{color:#BAE6FD!important;margin-top:8px;font-weight:800;font-size:.95rem}
...
def detect_yellow_sector_roi(img: Image.Image, min_area: int = 1500) -> Optional[Dict[str, int]]:
    """Detecta automáticamente el recuadro auxiliar del informe Z-Logic.

    El PDF del equipo suele marcar el panel pequeño de la derecha con una
    banda amarilla. Esta función encuentra ese rectángulo y permite usarlo
    como sector principal, evitando digitalizar otras curvas o tablas.
    """
    rgb = np.array(img.convert("RGB"))
    r = rgb[:, :, 0].astype(int)
    g = rgb[:, :, 1].astype(int)
    b = rgb[:, :, 2].astype(int)
    yellow = (r > 170) & (g > 130) & (b < 140) & ((r - b) > 70) & ((g - b) > 45)
    ys, xs = np.where(yellow)
    if len(xs) < int(min_area):
        return None
    x0, x1 = int(xs.min()), int(xs.max())
    y0, y1 = int(ys.min()), int(ys.max())
    # Validación geométrica básica para evitar falsos positivos pequeños.
    if (x1 - x0) < 80 or (y1 - y0) < 80:
        return None
    return {"x_min": x0, "x_max": x1, "y_min": y0, "y_max": y1}


def yellow_top_trace_defaults(yellow_roi: Dict[str, int], img_w: int, img_h: int) -> Dict[str, int]:
    """Propone el recorte correcto: sector auxiliar, pero solo la curva dZ/dt superior.

    No toma las señales inferiores ni el bloque de texto de la derecha como señal.
    """
    x0, x1 = yellow_roi["x_min"], yellow_roi["x_max"]
    y0, y1 = yellow_roi["y_min"], yellow_roi["y_max"]
    ww = max(1, x1 - x0)
    hh = max(1, y1 - y0)
    return {
        "x_min": max(0, x0 + int(0.01 * ww)),
        "x_max": min(img_w, x1 - int(0.01 * ww)),
        "y_min": max(0, y0 + int(0.04 * hh)),
        "y_max": min(img_h, y0 + int(0.55 * hh)),
    }



def left_middle_dzdt_defaults(img_w: int, img_h: int) -> Dict[str, int]:
    """Recorte por defecto para el trazado grande del informe Z-Logic.

    Corresponde al sector izquierdo-medio del bloque inferior donde está la curva
    dZ/dt principal, evitando el panel derecho, la señal ECG superior,
    la señal fono inferior y los textos laterales. Las proporciones se adaptan
    al tamaño de renderizado del PDF.
    """
    return {
        "x_min": max(0, int(img_w * 0.045)),
        "x_max": min(img_w, int(img_w * 0.730)),
        "y_min": max(0, int(img_h * 0.715)),
        "y_max": min(img_h, int(img_h * 0.865)),
    }


def reference_panel_roi(img: Image.Image) -> Dict[str, int]:
    """Detecta o propone el panel derecho del PDF Z-Logic mostrado por el usuario.

    La app usa ese panel como sector de origen, pero para digitalizar toma
    solamente la banda superior donde está la curva dZ/dt de referencia.
    Si el borde resaltado no se detecta, usa proporciones fijas del formato
    estándar Z-Logic.
    """
    w, h = img.size
    detected = detect_yellow_sector_roi(img)
    if detected is not None:
        return detected
    return {
        "x_min": max(0, int(w * 0.745)),
        "x_max": min(w, int(w * 0.985)),
        "y_min": max(0, int(h * 0.255)),
        "y_max": min(h, int(h * 0.680)),
    }


def reference_panel_top_dzdt_defaults(img: Image.Image) -> Dict[str, int]:
    """Recorta solamente la curva superior dZ/dt dentro del panel derecho de referencia."""
    w, h = img.size
    panel = reference_panel_roi(img)
    x0, x1 = panel["x_min"], panel["x_max"]
    y0, y1 = panel["y_min"], panel["y_max"]
    ww = max(1, x1 - x0)
    hh = max(1, y1 - y0)
    return {
        "x_min": max(0, x0 + int(0.02 * ww)),
        "x_max": min(w, x0 + int(0.98 * ww)),
        "y_min": max(0, y0 + int(0.03 * hh)),
        "y_max": min(h, y0 + int(0.47 * hh)),
    }

def draw_roi_preview(img: Image.Image, roi: Dict[str, int], color: str = "yellow") -> Image.Image:
    out = img.copy().convert("RGB")
    dr = ImageDraw.Draw(out)
    dr.rectangle([roi["x_min"], roi["y_min"], roi["x_max"], roi["y_max"]], outline=color, width=max(3, int(out.size[0] * 0.003)))
    return out
...
def digitize_curve_from_image(
    ...
    mask_mode: str = "azul_morado",
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """Digitaliza la curva seleccionada dentro del ROI.

    Cambio clave v7:
    - Si se usa `mask_mode="azul_morado"`, sigue preferentemente el trazo azul/morado del panel Z-Logic,
      evitando grilla, texto y señales inferiores.
    - El modo `seguimiento_continuo` sigue un único trazo, comenzando cerca de la banda vertical elegida.
      Esto permite digitalizar solo el sector auxiliar y, dentro de él, la señal dZ/dt superior.
    """
    ...
    # Modo recomendado para Z-Logic: sigue la tinta azul/morada de la curva y evita grilla/textos negros.
    if mask_mode == "azul_morado":
        rr = crop_rgb[:, :, 0].astype(int)
        gg = crop_rgb[:, :, 1].astype(int)
        bb = crop_rgb[:, :, 2].astype(int)
        mask = (bb > rr + 6) & (bb > gg + 3) & (bb < 235) & (rr < 205) & (gg < 205)
        # Respaldo: si el PDF vino en escala de grises o mala calidad, vuelve a percentil de oscuridad.
        if int(mask.sum()) < 80:
            thr = np.percentile(crop, threshold_percentile)
            mask = crop <= thr
        else:
            thr = float(np.percentile(crop[mask], 50)) if np.any(mask) else float(np.percentile(crop, threshold_percentile))
    else:
        # Método general por oscuridad: útil en capturas monocromas, pero puede captar texto/grilla si el ROI es amplio.
        thr = np.percentile(crop, threshold_percentile)
        mask = crop <= thr
    ...
    if len(xs) < 12:
        raise ValueError("No se pudo digitalizar una curva suficiente. Ajuste el recorte al panel de referencia dZ/dt superior, aumente sensibilidad o aumente el salto máximo permitido.")
...
def ui_digitalizacion(user: Dict[str, Any]) -> None:
    st.header("Digitalizar curva dZ/dt del panel de referencia y entrenar cursores")
    st.markdown("<div class='card'>Flujo corregido: el usuario carga el <b>PDF original descargado del estudio</b> → la app propone por defecto el <b>panel de referencia del PDF</b> → digitaliza solamente la curva <b>dZ/dt superior</b> → coloca B, C, X e Y automáticamente → el médico corrige → se guardan los deltas para aprendizaje.</div>", unsafe_allow_html=True)
    ...
    w, h = img.size
    panel_roi = reference_panel_roi(img)
    st.image(draw_roi_preview(img, panel_roi, color="orange"), caption=f"Fuente cargada: {source_label} ({w} x {h} px). Panel de referencia detectado/propuesto.", use_container_width=True)

    with st.expander("1. Recorte de la curva dZ/dt del panel de referencia", expanded=True):
        st.caption("La app debe tomar el panel de referencia del PDF y digitalizar solamente la curva dZ/dt superior. No debe usar las señales inferiores ni el texto lateral.")
        modo_roi = st.radio(
            "Sector a digitalizar",
            [
                "Panel de referencia - curva dZ/dt superior (recomendado)",
                "Ajuste manual libre",
            ],
            index=0,
        )

        if modo_roi.startswith("Panel de referencia"):
            d_roi = reference_panel_top_dzdt_defaults(img)
            st.success("Recorte automático activo: curva dZ/dt superior del panel de referencia.")
        else:
            d_roi = reference_panel_top_dzdt_defaults(img)

        c1, c2, c3, c4 = st.columns(4)
        with c1: x_min = st.slider("X mínimo", 0, max(1, w - 2), int(d_roi["x_min"]))
        with c2: x_max = st.slider("X máximo", min(int(x_min) + 2, w), w, int(max(int(x_min)+2, d_roi["x_max"])))
        with c3: y_min = st.slider("Y mínimo", 0, max(1, h - 2), int(d_roi["y_min"]))
        with c4: y_max = st.slider("Y máximo", min(int(y_min) + 2, h), h, int(max(int(y_min)+2, d_roi["y_max"])))

        roi_preview = img.crop((int(x_min), int(y_min), int(x_max), int(y_max)))
        st.image(roi_preview, caption="Vista previa: debe verse SOLO la curva dZ/dt superior del panel de referencia", use_container_width=True)

        c1, c2, c3 = st.columns(3)
        with c1:
            mask_label = st.selectbox("Máscara de trazo", ["Trazado azul/morado Z-Logic", "Oscuridad general"], index=0)
        with c2:
            threshold_percentile = st.slider("Sensibilidad si usa oscuridad general", 3, 70, 22, help="Solo se usa como respaldo si el trazado azul/morado no se detecta.")
        with c3:
        ...
        with c2:
            method_label = st.selectbox("Método", ["Seguimiento continuo del trazo seleccionado", "Mediana por columna (método previo)"], index=0)
        with c3:
            start_y_fraction = st.slider("Altura inicial esperada", 0.05, 0.95, 0.50, 0.01, help="0=arriba del recorte, 1=abajo. Para el panel de referencia suele funcionar cerca de 0.50.")
        with c4:
            max_jump_px = st.slider("Salto máximo permitido", 4, 80, 26, help="Aumentar si la curva tiene ascensos/descensos bruscos; bajar si salta a otra señal.")
        max_dense_fraction = st.slider("Excluir columnas demasiado densas por grilla/texto", 0.10, 0.90, 0.25, 0.05)
    ...
    mask_mode = "azul_morado" if mask_label.startswith("Trazado") else "oscuridad"
...
st.markdown(f"<div class='card'>La app conserva el ingreso por usuarios y suma el módulo de digitalización de la curva dZ/dt superior del panel de referencia del PDF Z-Logic, con corrección de cursores y aprendizaje progresivo por deltas auto-manual.<br><b>{APP_DEVELOPER}</b></div>", unsafe_allow_html=True)
