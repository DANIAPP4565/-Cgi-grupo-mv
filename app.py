...
def ui_digitalizacion(user: Dict[str, Any]) -> None:
    st.header("Digitalizar curva dZ/dt del sector izquierdo medio y entrenar cursores")
    st.markdown("<div class='card'>Flujo corregido: el usuario carga el <b>PDF original descargado del estudio</b> → la app propone por defecto el <b>sector izquierdo medio</b> del trazado grande → digitaliza la curva <b>dZ/dt</b> principal → coloca B, C, X e Y automáticamente → el médico corrige → se guardan los deltas para aprendizaje.</div>", unsafe_allow_html=True)
    ...
    w, h = img.size
    yellow_roi = detect_yellow_sector_roi(img)
    if yellow_roi:
        st.image(draw_roi_preview(img, yellow_roi, color="yellow"), caption=f"Fuente cargada: {source_label} ({w} x {h} px). Recorte auxiliar detectado automáticamente.", use_container_width=True)
    else:
        st.image(img, caption=f"Fuente cargada: {source_label} ({w} x {h} px)", use_container_width=True)

    with st.expander("1. Recorte de la curva dZ/dt del sector izquierdo medio", expanded=True):
        st.caption("La app debe digitalizar la curva dZ/dt grande ubicada en el sector izquierdo-medio del trazado inferior. No debe usar el panel derecho ni las señales ECG/Fono.")
        modo_roi = st.radio(
            "Sector a digitalizar",
            [
                "Sector izquierdo medio - curva dZ/dt grande (recomendado)",
                "Ajuste manual libre",
            ],
            index=0,
        )

        if modo_roi.startswith("Sector izquierdo"):
            d_roi = left_middle_dzdt_defaults(w, h)
            st.success("Recorte automático activo: sector izquierdo-medio de la curva dZ/dt grande.")
        else:
            d_roi = {"x_min": int(w * 0.045), "x_max": int(w * 0.730), "y_min": int(h * 0.715), "y_max": int(h * 0.865)}
        ...
        st.image(roi_preview, caption="Vista previa: debe verse SOLO la curva dZ/dt grande del sector izquierdo-medio", use_container_width=True)
        ...
        with c3:
            start_y_fraction = st.slider("Altura inicial esperada", 0.05, 0.95, 0.50, 0.01, help="0=arriba del recorte, 1=abajo. Para el sector izquierdo-medio suele funcionar cerca de 0.50.")
        with c4:
...
st.markdown(f"<div class='card'>La app conserva el ingreso por usuarios y suma el módulo de digitalización de la curva dZ/dt del sector izquierdo-medio del PDF Z-Logic, con corrección de cursores y aprendizaje progresivo por deltas auto-manual.<br><b>{APP_DEVELOPER}</b></div>", unsafe_allow_html=True)
