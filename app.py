...
# Editor robusto por clic sobre imagen/gráfico. Se usa como método principal
# porque es más estable que los objetos Fabric en Streamlit Cloud.
try:
...
def draw_rois_with_guides(img: Image.Image, rois: dict, selected_key: str | None = None) -> Image.Image:
    """Dibuja rectángulos de ROI con manijas visibles para edición por clic."""
    out = img.copy().convert("RGB")
    d = ImageDraw.Draw(out)
    ...
    handle = max(8, w // 160)
    for key in SIGNALS:
        r = clamp_roi(rois[key], w, h)
        col = colors[key]
        width = thick + (2 if key == selected_key else 0)
        d.rectangle([r["x0"], r["y0"], r["x1"], r["y1"]], outline=col, width=width)
        # Zona semitransparente simulada con diagonales finas para no ocultar curvas.
        for x in range(r["x0"], r["x1"], max(18, w // 80)):
            d.line([(x, r["y0"]), (min(r["x1"], x + 18), min(r["y1"], r["y0"] + 18))], fill=col, width=1)
        # Manijas de borde y esquinas.
        cx = (r["x0"] + r["x1"]) // 2
        cy = (r["y0"] + r["y1"]) // 2
        handles = [
            (r["x0"], cy), (r["x1"], cy), (cx, r["y0"]), (cx, r["y1"]),
            (r["x0"], r["y0"]), (r["x1"], r["y0"]), (r["x0"], r["y1"]), (r["x1"], r["y1"]),
        ]
        for hx, hy in handles:
            d.rectangle([hx-handle//2, hy-handle//2, hx+handle//2, hy+handle//2], fill="white", outline=col, width=max(2, thick//2))
        label = f" {names[key]} "
        d.rectangle([r["x0"], max(0, r["y0"]-24), r["x0"]+10+len(label)*8, r["y0"]], fill="white", outline=col)
        d.text((r["x0"]+4, max(0, r["y0"]-21)), label, fill=col)
    return out


def update_roi_from_click(rois: dict, signal_key: str, action: str, x: int, y: int, w: int, h: int) -> dict:
    """Actualiza el ROI elegido usando un clic en la imagen."""
    new_rois = {k: clamp_roi(dict(v), w, h) for k, v in rois.items()}
    r = dict(new_rois[signal_key])
    min_w = max(8, int(w * 0.01))
    min_h = max(8, int(h * 0.01))
    x = int(max(0, min(x, w)))
    y = int(max(0, min(y, h)))
    if action == "Borde izquierdo":
        r["x0"] = min(x, r["x1"] - min_w)
    elif action == "Borde derecho":
        r["x1"] = max(x, r["x0"] + min_w)
    elif action == "Borde superior":
        r["y0"] = min(y, r["y1"] - min_h)
    elif action == "Borde inferior":
        r["y1"] = max(y, r["y0"] + min_h)
    elif action == "Esquina superior izquierda":
        r["x0"] = min(x, r["x1"] - min_w)
        r["y0"] = min(y, r["y1"] - min_h)
    elif action == "Esquina superior derecha":
        r["x1"] = max(x, r["x0"] + min_w)
        r["y0"] = min(y, r["y1"] - min_h)
    elif action == "Esquina inferior izquierda":
        r["x0"] = min(x, r["x1"] - min_w)
        r["y1"] = max(y, r["y0"] + min_h)
    elif action == "Esquina inferior derecha":
        r["x1"] = max(x, r["x0"] + min_w)
        r["y1"] = max(y, r["y0"] + min_h)
    elif action == "Mover centro":
        rw = r["x1"] - r["x0"]
        rh = r["y1"] - r["y0"]
        r["x0"] = int(round(x - rw / 2)); r["x1"] = int(round(x + rw / 2))
        r["y0"] = int(round(y - rh / 2)); r["y1"] = int(round(y + rh / 2))
    new_rois[signal_key] = clamp_roi(r, w, h)
    return new_rois


def click_select_rois(img: Image.Image, rois_default: dict, key_prefix: str = "roi_click") -> dict:
    """Editor principal de ROIs por clic directo sobre la imagen.

    No depende de arrastrar objetos Fabric. El usuario elige qué borde/esquina quiere
    mover y hace clic en el punto exacto del informe. Esto funciona de forma estable
    en Streamlit Cloud y en navegadores donde el canvas no toma el mouse.
    """
    w, h = img.size
    safe_key = _safe_canvas_key(key_prefix)
    state_key = f"{safe_key}_rois_click_state"
    ...
    rois = {k: clamp_roi(v, w, h) for k, v in st.session_state[state_key].items()}

    labels = {"dzdt": "dZ/dt / impedancia", "ecg": "ECG", "fono": "Fonocardiograma"}
    st.markdown("<div class='guide'><b>Editor gráfico de bordes:</b> elija señal y borde, luego haga clic directamente sobre la imagen donde debe quedar ese borde. Repita hasta ajustar los tres sectores.</div>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns([1.2, 1.6, 1.0])
    with c1:
        signal_key = st.radio("Señal a ajustar", SIGNALS, format_func=lambda k: labels[k], horizontal=False, key=f"{safe_key}_signal")
    with c2:
        action = st.selectbox(
            "Qué mover con el próximo clic",
            ["Borde izquierdo", "Borde derecho", "Borde superior", "Borde inferior", "Mover centro", "Esquina superior izquierda", "Esquina superior derecha", "Esquina inferior izquierda", "Esquina inferior derecha"],
            key=f"{safe_key}_action",
        )
    with c3:
        st.write("")
        if st.button("Restablecer sectores", key=f"{safe_key}_reset_click_rois"):
            st.session_state[state_key] = rois_default
            rois = {k: clamp_roi(v, w, h) for k, v in rois_default.items()}
            st.session_state.pop(f"{safe_key}_last_roi_click_sig", None)

    max_display_w = 1150
    scale = min(1.0, max_display_w / max(1, w))
    disp_w = max(1, int(w * scale))
    disp_h = max(1, int(h * scale))
    overlay = draw_rois_with_guides(img, rois, selected_key=signal_key)
    display_img = overlay.resize((disp_w, disp_h)).convert("RGB") if scale != 1.0 else overlay.convert("RGB")

    changed = False
    if streamlit_image_coordinates is not None:
        value = streamlit_image_coordinates(display_img, key=f"{safe_key}_roi_image_click")
        if value and isinstance(value, dict) and "x" in value and "y" in value:
            # IMPORTANTE: la firma del clic usa SOLO coordenadas.
            # Antes incluía señal/borde y al cambiar el selector reutilizaba el clic anterior,
            # por eso a veces se movía un borde equivocado.
            click_sig = f"{value.get('x')}|{value.get('y')}"
            last_key = f"{safe_key}_last_roi_click_sig"
            if st.session_state.get(last_key) != click_sig:
                st.session_state[last_key] = click_sig
                real_x = int(round(float(value["x"]) / scale))
                real_y = int(round(float(value["y"]) / scale))
                rois = update_roi_from_click(rois, signal_key, action, real_x, real_y, w, h)
                st.session_state[state_key] = rois
                changed = True
                st.success(f"Aplicado: {labels[signal_key]} · {action} en x={real_x}, y={real_y}.")
    else:
        st.image(display_img, caption="Instale streamlit-image-coordinates para ajustar los bordes haciendo clic sobre la imagen.", use_container_width=True)
        st.warning("Falta instalar streamlit-image-coordinates. Se habilita respaldo numérico.")
        st.session_state[state_key] = numeric_rois_fallback(img, rois, safe_key)
        return st.session_state[state_key]

    if changed:
        updated_overlay = draw_rois_with_guides(img, rois, selected_key=signal_key)
        updated_display = updated_overlay.resize((disp_w, disp_h)).convert("RGB") if scale != 1.0 else updated_overlay.convert("RGB")
        st.image(updated_display, caption="Resultado aplicado después del clic", use_container_width=True)

    st.caption("Los rectángulos activos se usan inmediatamente para digitalizar. Para cambiar un borde: elija señal + borde y haga clic en la imagen.")
    st.session_state[state_key] = rois
    return rois


def numeric_rois_fallback(img: Image.Image, current_rois: dict, safe_key: str) -> dict:
    """Respaldo numérico cuando no está disponible el componente de clic."""
    w, h = img.size
...
def canvas_select_rois(img: Image.Image, rois_default: dict, key_prefix: str = "roi_canvas") -> dict:
    """Permite ajustar los sectores desde la propia imagen.

    Método principal: clic sobre imagen con streamlit-image-coordinates.
    Método alternativo: respaldo numérico si falta el componente.
    """
    w, h = img.size
    safe_key = _safe_canvas_key(key_prefix)
    if streamlit_image_coordinates is not None:
        return click_select_rois(img, rois_default, key_prefix=f"{safe_key}_click")
    st.info("Modo seguro: edite los bordes de cada sector con campos numéricos. Para edición gráfica instale streamlit-image-coordinates.")
    current = {k: clamp_roi(v, w, h) for k, v in rois_default.items()}
    return numeric_rois_fallback(img, current, safe_key)
...
def graph_adjust_cursors(chart_meta: dict, cursor_x: dict, key_prefix: str = "cursor_canvas") -> dict:
    """Permite mover QRS, B, C, X e Y con clic directo sobre el gráfico.

    El cursor elegido se ubica en la coordenada X del clic. Es más robusto que
    arrastrar líneas Fabric dentro de Streamlit Cloud y evita que el gráfico se
    reinicie o ignore el mouse.
    """
    ...
    disp_h = max(1, int(h * scale))
    safe_key = _safe_canvas_key(key_prefix)
    state_key = f"{safe_key}_cursor_positions_click_state"

    incoming = {c: float(min(max(cursor_x.get(c, chart_meta["x0"]), chart_meta["x0"]), chart_meta["x1"])) for c in CURSORS}
    if state_key not in st.session_state:
        st.session_state[state_key] = incoming
    positions = {c: float(min(max(st.session_state[state_key].get(c, incoming[c]), chart_meta["x0"]), chart_meta["x1"])) for c in CURSORS}

    st.markdown("<div class='guide'><b>Editor gráfico de cursores:</b> elija el cursor y haga clic sobre el gráfico en la posición deseada. QRS va sobre ECG; B, C, X e Y sobre dZ/dt.</div>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns([1.5, 2.5, 1.0])
    with c1:
        selected = st.radio("Cursor a mover", CURSORS, horizontal=True, key=f"{safe_key}_selected_cursor")
    with c2:
        st.markdown(" ".join([f"<span style='display:inline-block;margin-right:12px;color:{CURSOR_COLORS[c]};font-weight:800'>{c}</span>" for c in CURSORS]), unsafe_allow_html=True)
    with c3:
        if st.button("Restablecer cursores", key=f"{safe_key}_reset_cursors"):
            st.session_state[state_key] = incoming
            positions = dict(incoming)
            st.session_state.pop(f"{safe_key}_last_cursor_click_sig", None)

    # Reconstruye una vista con las posiciones vigentes antes del clic.
    preview_manual = {c: {"x": positions[c], "y": 0.5} for c in CURSORS}
    # Si build_curve_chart ya generó una imagen con posiciones iniciales, igualmente usamos esa imagen;
    # las posiciones se ven actualizadas en el gráfico final que se muestra debajo después de cada clic.
    display_img = img.resize((disp_w, disp_h)).convert("RGB") if scale != 1.0 else img.convert("RGB")

    changed = False
    if streamlit_image_coordinates is not None:
        value = streamlit_image_coordinates(display_img, key=f"{safe_key}_cursor_image_click")
        if value and isinstance(value, dict) and "x" in value:
            # IMPORTANTE: no incluir el cursor seleccionado en la firma.
            # Si se cambia QRS/B/C/X/Y, el componente conserva el último clic;
            # con esta protección no se aplica ese clic viejo al cursor nuevo.
            click_sig = f"{value.get('x')}|{value.get('y')}"
            last_key = f"{safe_key}_last_cursor_click_sig"
            if st.session_state.get(last_key) != click_sig:
                st.session_state[last_key] = click_sig
                original_px = float(value["x"]) / scale
                new_x = canvas_px_to_data_x(original_px, chart_meta)
                positions[selected] = float(new_x)
                st.session_state[state_key] = positions
                changed = True
                st.success(f"Cursor {selected} ubicado en x={new_x:.1f}.")
    else:
        st.image(display_img, caption="Instale streamlit-image-coordinates para mover cursores haciendo clic sobre el gráfico.", use_container_width=True)
        st.warning("Falta instalar streamlit-image-coordinates. Se habilita respaldo numérico.")
        cols = st.columns(5)
    ...
    st.caption("Después de cada clic, la app actualiza el cursor elegido y recalcula el gráfico final y el Excel de deltas.")
    ...
    with tab2:
        ...
        if uploaded2 is not None or use_last:
            try:
                ...
                st.markdown("#### 1) Ajustar sectores gráficos desde la imagen")
                ...
                if dzdt.empty:
                    ...
                    cursor_key_prefix = f"cursor_canvas_{source2}_{page2}_{preset}"
                    cursor_click_state_key = f"{_safe_canvas_key(cursor_key_prefix)}_cursor_positions_click_state"
                    default_manual_x = {c: float(min(max(auto[c]["x"], x0c), x1c)) for c in CURSORS}
                    manual_x_init = st.session_state.get(cursor_click_state_key, st.session_state.get(cursor_state_key, default_manual_x))
                    ...
                    st.markdown("#### 2) Mover cursores desde el gráfico")
                    adjusted_x = graph_adjust_cursors(chart_meta, manual_x_init, key_prefix=cursor_key_prefix)
                    manual_x_final = {cur: float(min(max(adjusted_x.get(cur, auto[cur]["x"]), x0c), x1c)) for cur in CURSORS}
                    usar_respaldo = st.checkbox("Mostrar respaldo numérico de cursores", value=False, key=f"show_cursor_numeric_{source2}_{page2}_{preset}")
                    if usar_respaldo:
                        st.caption("Úselo solo si el clic sobre el gráfico no está disponible en el navegador.")
                        cols = st.columns(5)
