...
# ============================================================
# streamlit-drawable-canvas 0.9.3 quedó archivado y llama internamente a
# streamlit.elements.image.image_to_url con la firma antigua:
#   image_to_url(image, width, clamp, channels, output_format, image_id)
# En Streamlit reciente la función fue movida a image_utils y ahora espera
#   image_to_url(image, layout_config, clamp, channels, output_format, image_id)
# Si se asigna la función nueva directamente, el parámetro width llega como int
# donde Streamlit espera layout_config.width y aparece:
#   AttributeError: 'int' object has no attribute 'width'
# Este wrapper traduce la firma antigua a la nueva y mantiene el selector gráfico.
    ...
    try:
        ...
        def _compat_image_to_url(
            ...
            **kwargs,
            ...
            # Soporta llamadas nuevas por si otro componente usa keywords.
            if "layout_config" in kwargs:
                return _new_image_to_url(
                    image,
                    layout_config=kwargs.get("layout_config"),
                    clamp=kwargs.get("clamp", clamp),
                    channels=kwargs.get("channels", channels),
                    output_format=kwargs.get("output_format", output_format),
                    image_id=kwargs.get("image_id", image_id),
                )

            # Traducción de la llamada antigua usada por streamlit-drawable-canvas.
            try:
                ...
                # Respaldo para Streamlit viejo, donde image_utils aún conserva la firma antigua.
                return _new_image_to_url(image, width, clamp, channels, output_format, image_id)

        # Se asigna siempre, no solo cuando falta, porque puede existir una función
        # importada con firma nueva incompatible con streamlit-drawable-canvas.
...
def canvas_select_rois(img: Image.Image, rois_default: dict, key_prefix: str = "roi_canvas") -> dict:
    ...
    Si streamlit-drawable-canvas no está instalado, conserva una alternativa
    robusta con campos numéricos, sin volver a usar barras deslizantes X/Y.
    """
    w, h = img.size
    ...
    st.caption("Ajuste los sectores desde la imagen: arrastre cada rectángulo o mueva sus bordes para que contenga solamente la señal correspondiente.")
    ...
    if st_canvas is not None:
        try:
            ...
            for i, key in enumerate(SIGNALS):
                ...
                left = float(obj.get("left", rois_default[key]["x0"] * scale))
                top = float(obj.get("top", rois_default[key]["y0"] * scale))
                width = float(obj.get("width", (rois_default[key]["x1"] - rois_default[key]["x0"]) * scale)) * float(obj.get("scaleX", 1.0))
                height = float(obj.get("height", (rois_default[key]["y1"] - rois_default[key]["y0"]) * scale)) * float(obj.get("scaleY", 1.0))
                rois[key] = clamp_roi({
            ...
            st.warning(
                "No se pudo abrir el selector gráfico de bordes por incompatibilidad de versión. "
                "La app continúa en modo seguro con edición de bordes numéricos. "
                f"Detalle técnico: {type(e).__name__}: {e}"
            )

    st.info("Modo seguro: edite los bordes de cada sector con campos numéricos. No se usan barras X/Y y la app no se detiene.")
    rois = {}
    st.image(draw_rois(img, rois_default), caption="Sectores iniciales sugeridos", use_container_width=True)
    for key in SIGNALS:
        b = rois_default[key]
        st.markdown(f"**{names[key]}**")
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            x0 = st.number_input(f"Borde izquierdo {key}", 0, max(1, w - 3), int(b["x0"]), key=f"{key_prefix}_{key}_x0_num")
        with c2:
            x1 = st.number_input(f"Borde derecho {key}", int(x0) + 3, w, int(max(int(x0) + 3, b["x1"])), key=f"{key_prefix}_{key}_x1_num")
        with c3:
            y0 = st.number_input(f"Borde superior {key}", 0, max(1, h - 3), int(b["y0"]), key=f"{key_prefix}_{key}_y0_num")
        with c4:
            y1 = st.number_input(f"Borde inferior {key}", int(y0) + 3, h, int(max(int(y0) + 3, b["y1"])), key=f"{key_prefix}_{key}_y1_num")
