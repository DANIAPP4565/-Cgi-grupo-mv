...
def _selected_plotly_point(event) -> dict | None:
    """Extrae el primer punto seleccionado devuelto por st.plotly_chart(on_select='rerun')."""
    try:
        if event is None:
            return None
...
def _blank_plotly_roi_fig(img: Image.Image, rois: dict, selector_x: list, selector_y: list, title: str) -> "go.Figure":
    """Crea una figura Plotly con la imagen como fondo, ROI dibujados y puntos seleccionables."""
    arr = np.asarray(img.convert("RGB"))
    h, w = arr.shape[:2]
    fig = go.Figure()
    fig.add_trace(go.Image(z=arr, hoverinfo="skip", name="imagen"))
    colors = {"dzdt": "royalblue", "ecg": "seagreen", "fono": "darkorange"}
    labels = {"dzdt": "dZ/dt", "ecg": "ECG", "fono": "Fono"}
    for key in SIGNALS:
        r = clamp_roi(rois[key], w, h)
        fig.add_shape(
            type="rect",
            x0=r["x0"], x1=r["x1"], y0=r["y0"], y1=r["y1"],
            line=dict(color=colors[key], width=4), fillcolor="rgba(0,0,0,0)",
        )
        fig.add_annotation(
            x=r["x0"], y=r["y0"], text=labels[key], showarrow=False,
            xanchor="left", yanchor="bottom", font=dict(color=colors[key], size=14),
            bgcolor="rgba(255,255,255,0.70)",
        )
    if selector_x and selector_y:
        fig.add_trace(go.Scatter(
            x=selector_x, y=selector_y, mode="markers", name="PUNTOS PARA SELECCIONAR",
            marker=dict(size=8, color="crimson", opacity=0.70),
            hovertemplate="x=%{x:.0f}<br>y=%{y:.0f}<extra>Seleccionar</extra>",
            selected=dict(marker=dict(size=12, color="black", opacity=1.0)),
            unselected=dict(marker=dict(opacity=0.35)),
        ))
    fig.update_layout(
        title=title,
        dragmode="select",
        height=min(780, max(420, int(h * min(1.0, 1150 / max(1, w))) + 80)),
        margin=dict(l=10, r=10, t=50, b=10),
        showlegend=False,
        newselection=dict(line=dict(color="crimson", width=2)),
    )
    fig.update_xaxes(range=[0, w], showgrid=False, zeroline=False, visible=False)
    fig.update_yaxes(range=[h, 0], showgrid=False, zeroline=False, visible=False, scaleanchor="x", scaleratio=1)
    return fig


def _roi_selector_points(roi: dict, w: int, h: int, action: str) -> tuple[list, list]:
    """Genera puntos seleccionables sobre la imagen según el borde que se quiere mover."""
    r = clamp_roi(roi, w, h)
    cx = int(round((r["x0"] + r["x1"]) / 2))
    cy = int(round((r["y0"] + r["y1"]) / 2))
    step_x = max(4, int(w / 220))
    step_y = max(4, int(h / 220))
    if action in ["Borde izquierdo", "Borde derecho"]:
        xs = list(range(0, w + 1, step_x))
        ys = [cy] * len(xs)
        return xs, ys
    if action in ["Borde superior", "Borde inferior"]:
        ys = list(range(0, h + 1, step_y))
        xs = [cx] * len(ys)
        return xs, ys
    # Centro o esquinas: grilla liviana alrededor de la imagen completa.
    xs, ys = [], []
    for yy in range(0, h + 1, max(12, step_y * 3)):
        for xx in range(0, w + 1, max(12, step_x * 3)):
            xs.append(xx); ys.append(yy)
    return xs, ys


def _apply_roi_point(roi: dict, point: dict, action: str, w: int, h: int) -> dict:
    """Aplica el punto seleccionado al ROI activo."""
    r = clamp_roi(roi, w, h)
    x = int(round(float(point.get("x", (r["x0"] + r["x1"]) / 2))))
    y = int(round(float(point.get("y", (r["y0"] + r["y1"]) / 2))))
    x = max(0, min(w, x)); y = max(0, min(h, y))
    min_w = 8
    min_h = 8
    if action == "Borde izquierdo":
        r["x0"] = min(max(0, x), r["x1"] - min_w)
    elif action == "Borde derecho":
        r["x1"] = max(min(w, x), r["x0"] + min_w)
    elif action == "Borde superior":
        r["y0"] = min(max(0, y), r["y1"] - min_h)
    elif action == "Borde inferior":
        r["y1"] = max(min(h, y), r["y0"] + min_h)
    elif action == "Centro":
        ww = r["x1"] - r["x0"]
        hh = r["y1"] - r["y0"]
        r["x0"] = int(round(x - ww / 2)); r["x1"] = r["x0"] + ww
        r["y0"] = int(round(y - hh / 2)); r["y1"] = r["y0"] + hh
    elif action == "Esquina superior izquierda":
        r["x0"] = min(max(0, x), r["x1"] - min_w); r["y0"] = min(max(0, y), r["y1"] - min_h)
    elif action == "Esquina superior derecha":
        r["x1"] = max(min(w, x), r["x0"] + min_w); r["y0"] = min(max(0, y), r["y1"] - min_h)
    elif action == "Esquina inferior izquierda":
        r["x0"] = min(max(0, x), r["x1"] - min_w); r["y1"] = max(min(h, y), r["y0"] + min_h)
    elif action == "Esquina inferior derecha":
        r["x1"] = max(min(w, x), r["x0"] + min_w); r["y1"] = max(min(h, y), r["y0"] + min_h)
    return clamp_roi(r, w, h)


def canvas_select_rois(img: Image.Image, rois_default: dict, key_prefix: str = "roi_canvas") -> dict:
    """Editor gráfico estable de ROI usando Plotly nativo de Streamlit.

    No usa streamlit-drawable-canvas ni streamlit-image-coordinates. El usuario
    selecciona puntos directamente sobre la imagen y confirma el cambio; esto
    evita el error de SessionInfo y no depende de componentes externos.
    """
    w, h = img.size
    ...
    pending_key = f"{safe_key}_pending_roi_point"
    ...
    c1, c2, c3 = st.columns([1.15, 1.6, 0.8])
    with c1:
        signal = st.radio("Señal a corregir", SIGNALS, format_func=lambda x: names[x], horizontal=True, key=f"{safe_key}_roi_signal")
    with c2:
        action = st.selectbox(
            "Qué desea mover desde la imagen",
            ["Borde izquierdo", "Borde derecho", "Borde superior", "Borde inferior", "Centro",
             "Esquina superior izquierda", "Esquina superior derecha", "Esquina inferior izquierda", "Esquina inferior derecha"],
            key=f"{safe_key}_roi_action",
        )
    with c3:
        if st.button("Restablecer", key=f"{safe_key}_reset_rois"):
            st.session_state[state_key] = rois_default
            st.session_state.pop(pending_key, None)
            rois = rois_default

    if go is None:
        st.warning("Falta plotly. Agregue plotly al requirements.txt para mover bordes desde la imagen.")
        st.image(draw_rois(img, rois), caption="Sectores actuales", use_container_width=True)
        return rois

    selector_x, selector_y = _roi_selector_points(rois[signal], w, h, action)
    fig = _blank_plotly_roi_fig(
        img, rois, selector_x, selector_y,
        f"Ajuste gráfico de bordes: {names[signal]} · {action}. Seleccione un punto rojo y luego confirme."
    )
    event = st.plotly_chart(
        fig,
        use_container_width=True,
        key=f"{safe_key}_roi_plot_{signal}_{action}",
        on_select="rerun",
        selection_mode="points",
        config={"displaylogo": False, "scrollZoom": True},
    )
    pt = _selected_plotly_point(event)
    if pt is not None:
        st.session_state[pending_key] = {"signal": signal, "action": action, "x": float(pt.get("x", 0)), "y": float(pt.get("y", 0))}

    pending = st.session_state.get(pending_key)
    if pending and pending.get("signal") == signal and pending.get("action") == action:
        st.info(f"Punto seleccionado: x={pending['x']:.0f}, y={pending['y']:.0f}. Presione Aplicar para mover {action.lower()} de {names[signal]}.")
        if st.button("Aplicar punto seleccionado al borde", type="primary", key=f"{safe_key}_apply_roi_point"):
            rois[signal] = _apply_roi_point(rois[signal], pending, action, w, h)
            st.session_state[state_key] = rois
            st.session_state.pop(pending_key, None)
            st.success("Borde aplicado. El recorte activo ya quedó actualizado.")
    else:
        st.caption("Active la herramienta de selección del gráfico si fuera necesario y seleccione un punto rojo sobre la imagen.")

    st.image(draw_rois(img, rois), caption="Sectores activos que se usarán para digitalizar", use_container_width=True)
    st.markdown(
        "<div class='ok'><b>Modo gráfico estable:</b> la corrección se hace desde la imagen mediante selección de puntos y confirmación. "
        "No usa arrastre externo, por eso evita el error SessionInfo de Streamlit Cloud.</div>",
        unsafe_allow_html=True,
    )
    return rois
...
def graph_adjust_cursors(chart_meta: dict, cursor_x: dict, key_prefix: str = "cursor_canvas") -> dict:
    """Mueve QRS, B, C, X, Y desde el gráfico usando selección Plotly nativa.

    El usuario elige un cursor, selecciona un punto sobre la línea roja del gráfico
    y confirma. No depende de streamlit-drawable-canvas ni image-coordinates.
    """
    img = Image.open(io.BytesIO(chart_meta["image_bytes"])).convert("RGB")
    w, h = img.size
    safe_key = _safe_canvas_key(key_prefix)
    pending_key = f"{safe_key}_pending_cursor_point"
    state_key = f"{safe_key}_cursor_positions"
    ...
    if state_key in st.session_state:
        try:
            ...
            for c in CURSORS:
                if c in saved:
                    positions[c] = float(min(max(saved[c], chart_meta["x0"]), chart_meta["x1"]))
        except Exception:
            pass

    target = st.radio("Cursor a mover desde el gráfico", CURSORS, horizontal=True, key=f"{safe_key}_target_cursor")
    st.markdown(
        " ".join([f"<span style='display:inline-block;margin-right:12px;color:{CURSOR_COLORS[c]};font-weight:800'>{c}</span>" for c in CURSORS]),
        unsafe_allow_html=True,
    )

    if go is None:
        st.warning("Falta plotly. Agregue plotly al requirements.txt para mover cursores desde el gráfico.")
        st.image(img, caption="Gráfico actual", use_container_width=True)
        return positions

    arr = np.asarray(img.convert("RGB"))
    fig = go.Figure()
    fig.add_trace(go.Image(z=arr, hoverinfo="skip", name="curvas"))
    # Puntos rojos seleccionables en el área de datos del gráfico. El eje X está en pixeles de la imagen,
    # y luego se convierte al X real de la curva con canvas_px_to_data_x.
    p0 = float(chart_meta.get("data_x0_pix", 0))
    p1 = float(chart_meta.get("data_x1_pix", w))
    if p1 < p0:
        p0, p1 = p1, p0
    xs = np.linspace(max(0, p0), min(w, p1), 260)
    # Ubicamos la regla roja sobre la señal que corresponde: ECG para QRS, dZ/dt para B/C/X/Y.
    y_rule = h * (0.47 if target == "QRS" else 0.25)
    ys = np.full_like(xs, y_rule)
    fig.add_trace(go.Scatter(
        x=xs, y=ys, mode="markers", name="PUNTOS PARA SELECCIONAR",
        marker=dict(size=7, color="crimson", opacity=0.70),
        hovertemplate="x gráfico=%{x:.0f}<extra>Seleccionar cursor</extra>",
        selected=dict(marker=dict(size=12, color="black", opacity=1.0)),
        unselected=dict(marker=dict(opacity=0.35)),
    ))
    # Líneas verticales actuales.
    for c in CURSORS:
        px = data_x_to_canvas_px(positions[c], chart_meta)
        fig.add_shape(type="line", x0=px, x1=px, y0=0, y1=h, line=dict(color=CURSOR_COLORS.get(c, "black"), width=3, dash="dash"))
        fig.add_annotation(x=px, y=20, text=c, showarrow=False, font=dict(color=CURSOR_COLORS.get(c, "black"), size=14), bgcolor="rgba(255,255,255,0.75)")
    fig.update_layout(
        title=f"Mover cursor {target}: seleccione un punto rojo en la posición deseada y confirme",
        dragmode="select",
        height=min(690, max(420, int(h * min(1.0, 1200 / max(1, w))) + 80)),
        margin=dict(l=10, r=10, t=55, b=10),
        showlegend=False,
        newselection=dict(line=dict(color="crimson", width=2)),
    )
    fig.update_xaxes(range=[0, w], visible=False, showgrid=False, zeroline=False)
    fig.update_yaxes(range=[h, 0], visible=False, showgrid=False, zeroline=False, scaleanchor="x", scaleratio=1)

    event = st.plotly_chart(
        fig,
        use_container_width=True,
        key=f"{safe_key}_cursor_plot_{target}",
        on_select="rerun",
        selection_mode="points",
        config={"displaylogo": False, "scrollZoom": True},
    )
    pt = _selected_plotly_point(event)
    if pt is not None:
        xpix = float(pt.get("x", data_x_to_canvas_px(positions[target], chart_meta)))
        new_x = canvas_px_to_data_x(xpix, chart_meta)
        st.session_state[pending_key] = {"cursor": target, "xpix": xpix, "x": float(new_x)}

    pending = st.session_state.get(pending_key)
    if pending and pending.get("cursor") == target:
        st.info(f"Punto seleccionado para {target}: x={pending['x']:.1f}. Presione Aplicar para mover el cursor.")
        if st.button("Aplicar punto seleccionado al cursor", type="primary", key=f"{safe_key}_apply_cursor"):
            positions[target] = float(pending["x"])
            st.session_state[state_key] = positions
            st.session_state.pop(pending_key, None)
            st.success(f"Cursor {target} actualizado desde el gráfico.")
    else:
        st.caption("Seleccione un punto rojo sobre el gráfico. QRS se corrige sobre ECG; B, C, X e Y sobre dZ/dt.")
