...
# - Digitalizacion de curva de impedancia desde imagen de estudio real
...
def digitize_curve_from_image(
    ...
    if len(xs) < 12:
        raise ValueError("No se pudo digitalizar una curva suficiente. Ajuste el recorte o el umbral de detección.")

    x = np.array(xs, dtype=float)
    y_pix = np.array(ys, dtype=float)
    y_s = smooth_signal(y_pix, window=max(5, int(len(y_pix) * 0.015)))
    ...
    quality = {
        ...
        "coverage_fraction": float(len(df) / max(1, (x1 - x0))),
...
def ui_digitalizacion(user: Dict[str, Any]) -> None:
    st.header("Digitalizar curva de impedancia y entrenar colocación de cursores")
    st.markdown("<div class='card'>Flujo: cargar imagen del estudio real → recortar la zona de la curva → la app digitaliza la señal → coloca B, C, X e Y automáticamente → el médico corrige → se guardan los deltas para aprendizaje.</div>", unsafe_allow_html=True)
    ...
    uploaded = st.file_uploader("Cargar imagen del estudio CGI (PNG, JPG, JPEG)", type=["png", "jpg", "jpeg"])
    use_global_learning = st.checkbox("Aplicar aprendizaje acumulado global para ajustar cursores automáticos", value=True)
    if uploaded is None:
        st.info("Cargue una imagen o captura donde se vea claramente la curva dZ/dt para iniciar la digitalización.")
        return

    img = Image.open(uploaded).convert("RGB")
    w, h = img.size
    st.image(img, caption=f"Imagen cargada: {uploaded.name} ({w} x {h} px)", use_container_width=True)

    with st.expander("1. Recorte de región útil de la curva", expanded=True):
        st.caption("Ajuste el recorte para dejar principalmente la curva de impedancia. Evite textos, grillas intensas y otras curvas si es posible.")
        c1, c2, c3, c4 = st.columns(4)
        with c1: x_min = st.slider("X mínimo", 0, max(1, w - 2), int(w * 0.05))
        with c2: x_max = st.slider("X máximo", min(x_min + 2, w), w, int(w * 0.95))
        with c3: y_min = st.slider("Y mínimo", 0, max(1, h - 2), int(h * 0.10))
        with c4: y_max = st.slider("Y máximo", min(y_min + 2, h), h, int(h * 0.90))
        c1, c2, c3 = st.columns(3)
        with c1: threshold_percentile = st.slider("Sensibilidad de detección de trazo", 5, 80, 35, help="Más bajo detecta trazos oscuros estrictos; más alto incorpora más píxeles.")
        with c2: invert_y = st.checkbox("Invertir eje Y para que el ascenso sea positivo", value=True)
        with c3: min_pixels_col = st.slider("Píxeles mínimos por columna", 1, 8, 1)
    ...
    try:
        curve_df, q = digitize_curve_from_image(img, roi, threshold_percentile, invert_y, min_pixels_col)
    ...
    with c1:
        if st.button("Guardar curva, corrección manual y aprendizaje", type="primary"):
            sid = save_curve_session(int(user["id"]), patient_code, str(fecha_estudio), contexto, uploaded.name, (w, h), roi, auto, manual, {**metrics, **q, "learning_offsets_applied": offsets}, conclusion)
...
if user is None:
    _ = login_register_ui()  # evita que Streamlit muestre el objeto DeltaGenerator
    st.stop()
