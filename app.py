...
def detect_yellow_sector_roi(img: Image.Image, min_area: int = 1500) -> Optional[Dict[str, int]]:
    """Detecta automáticamente el recuadro amarillo del informe Z-Logic.
...
def yellow_top_trace_defaults(yellow_roi: Dict[str, int], img_w: int, img_h: int) -> Dict[str, int]:
    """Propone el recorte correcto: sector amarillo, pero solo la curva dZ/dt superior.
...
def digitize_curve_from_image(
    ...
    - El modo `seguimiento_continuo` sigue un único trazo, comenzando cerca de la banda vertical elegida.
      Esto permite digitalizar solo el sector amarillo y, dentro de él, la señal dZ/dt superior.
    """
...
def ui_digitalizacion(user: Dict[str, Any]) -> None:
    st.header("Digitalizar sector amarillo del PDF y entrenar cursores")
    st.markdown("<div class='card'>Flujo corregido: el usuario carga el <b>PDF original descargado del estudio</b> → la app detecta el <b>sector señalado en amarillo</b> → dentro de ese sector digitaliza solo la curva dZ/dt superior → coloca B, C, X e Y automáticamente → el médico corrige → se guardan los deltas para aprendizaje.</div>", unsafe_allow_html=True)
    ...
    if yellow_roi:
        st.image(draw_roi_preview(img, yellow_roi, color="yellow"), caption=f"Fuente cargada: {source_label} ({w} x {h} px). Recuadro amarillo detectado automáticamente.", use_container_width=True)
    else:
        st.image(img, caption=f"Fuente cargada: {source_label} ({w} x {h} px)", use_container_width=True)

    with st.expander("1. Sector amarillo y recorte de la curva dZ/dt superior", expanded=True):
        st.caption("La app debe digitalizar el sector señalado en amarillo. Dentro de ese panel, se toma solo la curva dZ/dt superior, no las señales inferiores ni los textos.")
        usar_amarillo = False
        if yellow_roi:
            usar_amarillo = st.checkbox("Usar sector amarillo detectado automáticamente", value=True)
            st.success("Sector amarillo detectado. Los valores de recorte se ajustan automáticamente al trazado dZ/dt superior dentro de ese sector.")
        else:
            st.warning("No se detectó automáticamente el recuadro amarillo. Ajuste manualmente el recorte sobre el panel amarillo.")

        if usar_amarillo and yellow_roi:
            d_roi = yellow_top_trace_defaults(yellow_roi, w, h)
        else:
            d_roi = {"x_min": int(w * 0.02), "x_max": int(w * 0.72), "y_min": int(h * 0.06), "y_max": int(h * 0.42)}
        ...
        st.image(roi_preview, caption="Vista previa: debe verse la curva dZ/dt superior del sector amarillo", use_container_width=True)
        ...
        with c3:
            start_y_fraction = st.slider("Altura inicial esperada", 0.05, 0.95, 0.62 if usar_amarillo else 0.38, 0.01, help="0=arriba del recorte, 1=abajo. Para el sector amarillo suele funcionar cerca de 0.60.")
        with c4:
...
st.markdown("<div class='card'>La app conserva el ingreso por usuarios y suma el módulo de digitalización del sector amarillo del PDF Z-Logic, con corrección de cursores y aprendizaje progresivo por deltas auto-manual.</div>", unsafe_allow_html=True)
