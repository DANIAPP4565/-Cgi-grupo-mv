...
def create_admin_user(username: str, password: str, full_name: str = "Administrador") -> Tuple[bool, str]:
    ...
    if len(username) < 3 or len(password) < 12:
        return False, "El usuario debe tener al menos 3 caracteres y la clave de administrador al menos 12."
    try:
        con = connect()
        con.execute(
            "INSERT INTO users(username,password_hash,full_name,matricula,provincia,role,active,created_at) VALUES(?,?,?,?,?,?,?,?)",
...
def set_user_password(username: str, password: str) -> Tuple[bool, str]:
    username = (username or "").strip()
    if len(password) < 12:
        return False, "La clave debe tener al menos 12 caracteres."
    con = connect()
    row = con.execute("SELECT id FROM users WHERE lower(username)=lower(?)", (username,)).fetchone()
    if not row:
        con.close()
...
def admin_password_ui(current_user: dict) -> None:
    ...
    with st.expander("Roles de usuarios", expanded=False):
        if all_users:
...
def login_ui() -> None:
    ...
    with t1:
        ...
        with c2:
            ...
            if not admin_exists():
                with st.expander("Crear administrador inicial", expanded=False):
                    setup_token_cfg = get_secret_value("CGI_SETUP_TOKEN", "")
                    st.caption("Use este bloque solo en la instalación inicial. Recomendado: definir CGI_SETUP_TOKEN en Secrets de Streamlit.")
                    ...
                    if st.button("Crear administrador inicial", type="primary", key="btn_setup_admin"):
                        if setup_token_cfg and not secrets.compare_digest(setup_token, setup_token_cfg):
                            st.error("Token de instalación incorrecto.")
                        elif not setup_token_cfg and setup_token.strip() != "CREAR-ADMIN":
                            st.error("Defina CGI_SETUP_TOKEN en Secrets o escriba CREAR-ADMIN para instalación local controlada.")
                        else:
                            if admin_pass_mode == "Generar automática":
                                new_admin_pass = generate_password(16)
                            else:
                                if init_pass != init_pass_2:
                                    st.error("Las claves no coinciden.")
                                    new_admin_pass = ""
                                else:
...
def graph_adjust_cursors(chart_meta: dict, cursor_x: dict, key_prefix: str = "cursor_canvas") -> dict:
    ...
    with st.expander("Respaldo numérico si el gráfico no responde", expanded=True):
...
def app_main() -> None:
    ...
    with tab4:
        if user.get("role") != "admin":
            ...
            with st.expander("Crear o redefinir clave de administrador", expanded=True):
