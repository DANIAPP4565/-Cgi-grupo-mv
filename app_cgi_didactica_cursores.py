...
# DE IMPEDANCIA - VERSION CORREGIDA Y ROBUSTA
# ============================================================

APP_TITLE = "App para Repositorio y aprendizaje en Cardiografía de Impedancia"
APP_SUBTITLE = "Corrección visual de cursores B, C, X e Y integrando ECG, dZ/dt y fonocardiograma"
...
def save_session(
    ...
    cur = c.execute(
        ...
        (
            ...
            source_name,
            page_number,
            json.dumps(rois, ensure_ascii=False),
...
def count_pdf_pages(pdf_bytes: bytes) -> int:
    if fitz is None:
        raise RuntimeError("Falta PyMuPDF. Agregue PyMuPDF al requirements.txt y redeploy.")
...
def open_uploaded_image(uploaded) -> Tuple[Image.Image, int, str]:
    ...
    if ext == ".pdf":
        ...
        with c1:
            default_page = min(2, max(1, n))
            page = st.number_input(
                "Página del PDF con ECG + dZ/dt + fonocardiograma",
                min_value=1,
                max_value=max(1, n),
                value=default_page,
                step=1,
...
def color_mask(crop_rgb: np.ndarray, mode: str) -> np.ndarray:
    ...
    if mode == "ecg_verde":
        mask = (g > r + 10) & (g > b + 4) & (g < 245) & (r < 225) & (b < 225)
    elif mode == "dzdt_azul":
        mask = (b > r + 5) & (b > g + 2) & (b < 250) & (r < 230) & (g < 230)
    elif mode == "fono_naranja":
        mask = (r > g + 5) & (g > b + 5) & (r > 110) & (b < 205)
    else:
...
def digitize_signal(img: Image.Image, roi: Dict[str, int], mode: str, min_points: int = 10) -> pd.DataFrame:
    ...
    if crop_h < 2 or crop_w < 2:
        return pd.DataFrame(columns=["x", "y_pixel", "y_smooth", "y_norm"])

    mask = color_mask(crop_rgb, mode)

    # Respaldo por oscuridad si el color no aparece bien en el PDF escaneado.
    if int(mask.sum()) < 80:
        gray = np.array(ImageOps.grayscale(Image.fromarray(crop_rgb.astype("uint8"))))
        mask = gray <= np.percentile(gray, 35)
...
def resample_to_common_x(df: pd.DataFrame, x_common: np.ndarray) -> np.ndarray:
    ...
    x, y = x[ok], y[ok]
    order = np.argsort(x)
    return np.interp(x_common, x[order], y[order], left=np.nan, right=np.nan)


def nearest_y(df: pd.DataFrame, x_value: float) -> float:
    if df.empty or "x" not in df or "y_norm" not in df:
        return 0.0
    arr = df["x"].to_numpy(float)
    if len(arr) == 0:
        return 0.0
    idx = int(np.nanargmin(np.abs(arr - float(x_value))))
...
def detect_guides(
    ...
    if not ecg.empty and "y_norm" in ecg:
        e = ecg.dropna(subset=["x", "y_norm"]).copy()
        if not e.empty:
            guide["qrs"] = float(e.iloc[int(np.argmax(e["y_norm"].to_numpy(float)))]["x"])

    if not fono.empty and "y_norm" in fono:
        f = fono.dropna(subset=["x", "y_norm"]).copy()
        if len(f) >= 5:
            ...
            for i, val in enumerate(above):
                ...
                if start is not None and ((not val) or i == len(above) - 1):
                    ...
                    start = None
            scored = []
            fx = f["x"].to_numpy(float)
            for a, b in groups:
                center = float(np.nanmean(fx[a:b]))
                scored.append(center)
    ...
    if not dzdt.empty and "y_norm" in dzdt:
        d = dzdt.dropna(subset=["x", "y_norm"]).copy()
        if len(d) >= 5:
...
def plot_three_signals(
    ...
    ax.plot(x_common, fo_y, linewidth=1.8, label="Fonocardiograma")
    ...
    for c in CURSORS:
        ax.axvline(float(auto[c]["x"]), linestyle=":", linewidth=1.4)
        ax.text(float(auto[c]["x"]), 2.18, f"{c} auto", rotation=90, ha="center", va="bottom", fontsize=8)
        ax.axvline(float(manual[c]["x"]), linestyle="--", linewidth=2.0)
        ax.text(float(manual[c]["x"]), 1.05, f"{c}", rotation=90, ha="center", va="bottom", fontsize=10, fontweight="bold")
...
def build_cursor_table(auto: Dict[str, Dict[str, float]], manual: Dict[str, Dict[str, float]], guide: Dict[str, float]) -> pd.DataFrame:
    ...
    for c in CURSORS:
        ...
        if c == "B":
            ...
            ref = "nadir sistólico; contrastar con S2/fonocardiograma"
        else:
            ref = "rebote diastólico posterior, si es visible"
        rows.append(
            {
                "Cursor": c,
                "Auto_x": round(float(auto[c]["x"]), 1),
                "Manual_x": round(mx, 1),
                "Delta_x": round(mx - float(auto[c]["x"]), 1),
...
def make_conclusion(df: pd.DataFrame, guide: Dict[str, float]) -> Tuple[Dict[str, float], str]:
    ...
    has_qrs = bool(np.isfinite(guide.get("qrs", np.nan)))
    has_s1 = bool(np.isfinite(guide.get("s1", np.nan)))
    has_s2 = bool(np.isfinite(guide.get("s2", np.nan)))
    metrics = {
        "error_medio_px": mae,
    ...
    conclusion = (
        ...
        "El cursor B se valida respecto del QRS y del inicio del ascenso de dZ/dt; C sobre el pico sistólico principal; "
...
st.markdown(
    """
    <div class='guide'>
    <b>Objetivo:</b> enseñar y corregir cursores integrando tres señales sincronizadas: ECG, curva dZ/dt y fonocardiograma. El fonocardiograma usa una línea horizontal simple para ubicar S1 y S2 como referencia visual.
    </div>
    """,
    unsafe_allow_html=True,
)
...
with tab1:
    ...
    if uploaded is None:
        ...
        with st.expander("1. Ajustar recortes de ECG, dZ/dt y fonocardiograma", expanded=True):
            st.markdown(
                "<div class='small'>Ajuste cada rectángulo hasta que contenga solo su señal. El eje X debe cubrir el mismo ciclo cardíaco en las tres bandas.</div>",
                unsafe_allow_html=True,
            )
            rois: Dict[str, Dict[str, int]] = {}
            ...
            for key in ["ecg", "dzdt", "fono"]:
                ...
                with c2:
                    x_low = min(x_min + 2, w)
                    x_max_default = max(x_low, min(base["x_max"], w))
                    x_max = st.slider(f"X máx {labels[key]}", x_low, w, x_max_default, key=f"{key}_x1")
                with c3:
                    y_min = st.slider(f"Y mín {labels[key]}", 0, max(1, h - 2), base["y_min"], key=f"{key}_y0")
                with c4:
                    y_low = min(y_min + 2, h)
                    y_max_default = max(y_low, min(base["y_max"], h))
                    y_max = st.slider(f"Y máx {labels[key]}", y_low, h, y_max_default, key=f"{key}_y1")
        ...
        with st.expander("2. Parámetros didácticos de detección", expanded=True):
            ...
            with c2:
                st.caption("La línea horizontal no intenta diagnosticar: ayuda a ver dónde aparecen los ruidos. Los cursores finales son siempre los corregidos manualmente.")
        ...
        if dzdt.empty:
            st.error("No se pudo digitalizar la curva dZ/dt. Ajuste el recorte azul, aumente la resolución del PDF o use una imagen con mayor contraste.")
            st.stop()
        if ecg.empty:
            st.warning("ECG no detectado con claridad. Puede corregir cursores, pero falta la referencia QRS.")
        if fono.empty:
            st.warning("Fonocardiograma no detectado con claridad. Puede corregir cursores, pero falta la referencia S1/S2.")

        x_min_common = max([rois[k]["x_min"] for k in rois])
        x_max_common = min([rois[k]["x_max"] for k in rois])
        if x_max_common <= x_min_common + 5:
            st.error("Los recortes no comparten un eje X suficiente. Alinee los tres recortes para que cubran el mismo ciclo cardíaco.")
            st.stop()

        auto, guide = detect_guides(ecg, dzdt, fono, x_min_common, x_max_common, fono_line)

        st.markdown(
            "<div class='ok'><b>Señales digitalizadas.</b> Corrija B, C, X e Y mirando simultáneamente ECG, dZ/dt y fonocardiograma.</div>",
            unsafe_allow_html=True,
        )
        ...
        with st.expander("3. Corrección manual de cursores", expanded=True):
            ...
            for i, c in enumerate(CURSORS):
                with cols[i]:
                    default_x = int(round(float(auto[c]["x"])))
                    default_x = min(max(default_x, min_slider), max_slider)
                    val = st.slider(f"Cursor {c}", min_slider, max_slider, default_x, key=f"manual_{c}")
        ...
        st.image(chart.getvalue(), caption="Vista didáctica sincronizada de las tres señales y cursores corregidos", use_container_width=True)
...
with tab2:
    ...
    st.markdown(
        """
        <div class='card'>
        <b>Qué hace esta versión:</b> digitaliza ECG, dZ/dt y fonocardiograma por recortes sincronizados; permite corregir manualmente B, C, X e Y; guarda la sesión; y exporta el aprendizaje a Excel.
        </div>
        """,
        unsafe_allow_html=True,
    )
